...
شغّليه محلياً:  python app.py
ثم افتحي:        http://127.0.0.1:5000
"""
...
def upload():
    ...
    if not name.lower().endswith((".xlsx", ".xls", ".csv")):
        return jsonify({"ok": False, "error": "الصيغة غير مدعومة. ارفعي ملف Excel‏ (.xlsx) أو CSV."}), 400
    try:
...
def api_filters():
    ...
    if not d:
        return jsonify({"ok": False, "error": "انتهت صلاحية البيانات. أعيدي رفع الملف."}), 400
